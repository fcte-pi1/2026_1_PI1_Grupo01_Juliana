...
//
// Usa as primitivas de movimentacao JA VALIDADAS na base de navegacao atual:
//   movimentacao_move_cell  -> anda 1 celula (retorna bool: true = avancou)
...
//
// Diferente da branch original, aqui o move_cell devolve sucesso/falha e um
// status (a base tem deteccao de travamento). Se o robo NAO conseguir avancar
// a celula (parede que o sensor nao pegou, roda presa), marcamos a parede a
// frente e o Flood Fill recalcula — a posicao (linha,coluna) nunca dessincroniza.
//
...
// cada celula, a leitura crua dos sensores e a decisao tomada.
...
// Pinos dos sensores IR (iguais aos de components/infrared/infrared.h).
#define IR_FRONT_PIN     GPIO_NUM_34   // frontal dedicado (pode nao estar instalado)
#define IR_FL_PIN        GPIO_NUM_26   // diagonal frontal-esquerda
#define IR_FR_PIN        GPIO_NUM_16   // diagonal frontal-direita
#define IR_L_PIN         GPIO_NUM_14   // lateral esquerda
#define IR_R_PIN         GPIO_NUM_13   // lateral direita

// O sensor frontal dedicado (GPIO 34) esta COMENTADO na infrared.h da base,
// entao por padrao a parede da frente e detectada pelos dois diagonais
// (FL E FR juntos). Se voce instalar/confiar no frontal, ponha 1 para tambem
// usar o GPIO 34.
#define USA_SENSOR_FRONTAL  0
...
{
    gpio_config_t cfg = {
        .pin_bit_mask = (1ULL << IR_FRONT_PIN) | (1ULL << IR_FL_PIN) |
                        (1ULL << IR_FR_PIN) | (1ULL << IR_L_PIN) | (1ULL << IR_R_PIN),
...
static void sentir_paredes(int l, int c, orientacao_t head)
{
    ...
    // Parede a frente: por padrao (USA_SENSOR_FRONTAL 0) so os dois diagonais
    // FL e FR juntos, porque o frontal dedicado (34) pode nao estar instalado.
    // FL ou FR sozinho = parede lateral, nao conta como frente.
    bool diagonais = ha_parede(IR_FL_PIN) && ha_parede(IR_FR_PIN);
#if USA_SENSOR_FRONTAL
    bool pf = ha_parede(IR_FRONT_PIN) || diagonais;
#else
    bool pf = diagonais;
#endif
    bool pd = ha_parede(IR_R_PIN);
    bool pe = ha_parede(IR_L_PIN);

    ESP_LOGI(TAG,
             "sensores brutos -> FRONT:%d FL:%d FR:%d L:%d R:%d | "
             "parede frente:%d esq:%d dir:%d",
             gpio_get_level(IR_FRONT_PIN), gpio_get_level(IR_FL_PIN),
             gpio_get_level(IR_FR_PIN), gpio_get_level(IR_L_PIN),
             gpio_get_level(IR_R_PIN), pf, pe, pd);
...
// Gira para a direcao desejada (caminho mais curto) e anda 1 celula.
// Retorna true se o robo realmente avancou; false se o move_cell falhou
// (nesse caso marca a parede a frente para o Flood Fill recalcular).
static bool executar_movimento(orientacao_t *head, int dir, int *l, int *c)
{
    ...
    switch (diff) {
        case 1: // direita
            ...
            break;
        case 2: // meia-volta (dois giros de 90 no mesmo sentido)
            movimentacao_turn_clws(&motorR, &motorL, &encoderR, &encoderL, &pose);
            movimentacao_turn_clws(&motorR, &motorL, &encoderR, &encoderL, &pose);
            break;
        case 3: // esquerda
    ...
    *head = (orientacao_t)dir;
    ...
    bool avancou = movimentacao_move_cell(
    ...
    // Nao avancou: ha (provavelmente) uma parede que os sensores nao pegaram,
    // ou a roda travou. Marca a parede a frente e mantem (linha,coluna) — o
    // Flood Fill vai recalcular e escolher outro vizinho no proximo passo.
    ESP_LOGW(TAG,
             "move_cell NAO avancou (status=%d) em (%d,%d) rumo %s — marcando parede",
             status, *l, *c, odometria_orientacao_string((orientacao_t)dir));
    marcar_parede(*l, *c, dir);
    bip(300, 120);
    return false;
}
