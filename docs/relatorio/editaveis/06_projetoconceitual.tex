...
Esta seção apresenta o projeto estrutural do \textit{micromouse} e do
labirinto de testes 4×4 desenvolvido pela equipe. São descritos os
...
% ------------------------------------------------------------------
% TABELA — Dimensões principais do chassi
% Preencha com os valores reais do seu CAD.
% ------------------------------------------------------------------
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcc}
        ...
        Comprimento total                 & ???                 & Limite máx. 250 mm           \\
        Largura total                     & ???                 & Limite máx. 250 mm           \\
        Altura total (sem componentes)    & ???                 & Sem restrição pela regra     \\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figuras/visao_geral.jpeg}
...
% ------------------------------------------------------------------
% FIGURA — Detalhe do módulo de parede (peça individual com cotas)
% ------------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figuras/Parede.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figuras/Chao.pdf}
...
% ------------------------------------------------------------------
% TEXTO — Decisões de modularidade
% Explique o sistema de encaixe, por que as dimensões da parede
% (1,2 cm de espessura) garantem estabilidade, e como a
% modularidade permite reconfigurar os labirintos para testes.
% ------------------------------------------------------------------

\subsubsection*{Decisões de Modularidade}

% >>> ESCREVA AQUI: explicação do sistema de encaixe e modularidade <<<
% Tópicos sugeridos:
%  - Tipo de encaixe (macho-fêmea, pinos, ranhuras etc.)
%  - Como a espessura de 12 mm contribui para a rigidez da parede
%  - Capacidade de reconfigurar o layout do labirinto para diferentes testes
%  - Facilidade de transporte e armazenamento

% ------------------------------------------------------------------
% TEXTO — Justificativa do material: madeira (MDF / compensado / etc.)
% ------------------------------------------------------------------
...
\begin{itemize}
    \item \textbf{Baixo custo e disponibilidade:}
          O MDF é facilmente encontrado em qualquer marcenaria ou loja de bricolagem, possuindo um custo-benefício excelente para a construção de dezenas de módulos de parede idênticos, viabilizando o projeto de um labirinto versátil.

    \item \textbf{Rigidez e estabilidade dimensional:}
          Sendo um painel de fibras de média densidade, ele não apresenta veios direcionais como a madeira maciça ou compensado, garantindo que os pinos de encaixe modular não sofram deformações desiguais, mantendo as larguras das células rigorosamente dentro da tolerância do padrão \textit{micromouse}.

    \item \textbf{Facilidade de fabricação e acabamento:}
          Por possuir uma superfície lisa, o MDF permite um excelente acabamento em pintura (necessário para a leitura do sensor IR nas cores branco e vermelho). Além disso, processos de usinagem CNC e corte a laser garantem precisão milimétrica nas juntas de encaixe dos módulos, garantindo montagem por \textit{press-fit} (encaixe por pressão) sem necessidade de cola, parafusos ou ferramentas.

    \item \textbf{Massa e praticidade:}
          Os blocos modulares em MDF possuem massa confortável para o manuseio rápido. Isso é vital, visto que um dos focos da modularidade é permitir a rápida reconfiguração (troca de geometria do labirinto) pelo operador durante os testes de validação dos algoritmos do robô.
\end{itemize}

% ------------------------------------------------------------------
% TEXTO — Acabamento superficial (pintura)
% Explique as cores exigidas pela regra e como foram aplicadas.
% ------------------------------------------------------------------

\subsubsection*{Acabamento Superficial}

Para satisfazer o regulamento, as faces laterais das paredes modulares receberam pintura em esmalte ou tinta acrílica branca, garantindo excelente reflectância para os emissores infravermelhos. O topo das paredes foi pintado de vermelho, demarcando visualmente os limites. Por fim, o tabuleiro (piso) principal recebeu acabamento em preto fosco para absorver a radiação IR e criar o contraste de leitura adequado aos sensores descendentes (quando presentes) e mitigar o espalhamento de luz horizontal.
