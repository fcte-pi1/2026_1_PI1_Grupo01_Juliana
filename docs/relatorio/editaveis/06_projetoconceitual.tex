...
% =============================================================
%  SEÇÃO 4.2 — ESTRUTURA
%  Projeto Conceitual do Produto | PI1 2026/1
%  Responsável: Equipe de Estrutura
%  Issue: Memorial Descritivo — Chassi (CAD) e Labirinto Modular
% =============================================================
...
% ==================================================================
%  4.2.1  CHASSI DO MICROMOUSE
% ==================================================================

\subsection{Chassi do \textit{Micromouse}}

% ------------------------------------------------------------------
% TEXTO — Visão geral do chassi
% Explique brevemente o que é o chassi, sua função no sistema e
% como ele foi concebido (requisitos de tamanho: ≤ 25 × 25 cm).
% ------------------------------------------------------------------

% >>> ESCREVA AQUI: visão geral do chassi (3–5 linhas) <<<
% Exemplo orientativo (apague e substitua pelo texto da equipe):
%
...
% comprimento e 25 cm de largura). O design prioriza ...


% ------------------------------------------------------------------
% FIGURA — Vista isométrica / renderização do chassi em CAD
% Substitua "chassi_isometrico" pelo nome real do arquivo em
% figuras/. Ajuste a largura conforme necessário (0.8\linewidth
% funciona bem para uma figura principal).
% ------------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\linewidth]{figuras/chassi_isometrico}
    \caption{Vista isométrica do chassi do \textit{micromouse} modelado
             em CAD. Dimensões em milímetros.}
    \label{fig:chassi_isometrico}
\end{figure}

% ------------------------------------------------------------------
% FIGURA — Vistas ortogonais com cotas (frontal, lateral, superior)
% Se tiver em um único arquivo, use apenas um \includegraphics.
% Se tiver em arquivos separados, use minipage como abaixo.
% ------------------------------------------------------------------
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/chassi_frontal}
    ...
    \begin{minipage}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/chassi_lateral}
    ...
    \begin{minipage}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/chassi_superior}
        \caption*{(c) Vista superior}
    \end{minipage}
    \caption{Vistas ortogonais do chassi com cotas principais (mm).}
    \label{fig:chassi_vistas}
\end{figure}

% ------------------------------------------------------------------
% FIGURA — Vista explodida (opcional, mas recomendada)
% Mostra a disposição de todos os componentes fixados ao chassi.
% Remova o bloco inteiro se não tiver essa vista.
% ------------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figuras/chassi_explodido}
    \caption{Vista explodida do chassi indicando a posição dos
             principais componentes: motores (1), rodas (2),
             placa ESP32 (3), bateria (4) e sensores IR (5).
             % Ajuste a legenda numerando os itens reais do seu projeto.
             }
...
% ------------------------------------------------------------------
% TEXTO — Disposição dos componentes
% Descreva onde cada subsistema está posicionado no chassi e
% por que essa disposição foi escolhida (centro de gravidade,
% acesso para manutenção, proximidade dos sensores com as paredes etc.)
% ------------------------------------------------------------------

\subsubsection*{Disposição dos Componentes}

% >>> ESCREVA AQUI: descrição da disposição dos componentes <<<
% Tópicos sugeridos:
%  - Posição dos motores e como influencia o giro no lugar
%  - Posição da bateria (impacta o centro de massa)
%  - Posição dos sensores IR (frontal, laterais)
%  - Posição da placa de controle (ESP32)
%  - Roteamento dos cabos / acesso para manutenção

% ------------------------------------------------------------------
% TEXTO — Justificativa do material: PLA
% ------------------------------------------------------------------

\subsubsection*{Justificativa do Material — PLA}

% >>> ESCREVA AQUI: justificativa técnica do PLA <<<
% Tópicos obrigatórios (conforme critérios da issue):
%  - Densidade do PLA vs. ABS/PETG (equilíbrio peso × resistência)
%  - Resistência mecânica suficiente para impactos com as paredes
%  - Facilidade de impressão 3D (temperatura, aderência)
%  - Custo e disponibilidade no mercado nacional
...
\begin{itemize}
    ...
    \item \textbf{Limitações conhecidas e mitigações adotadas:}
          % >>> ex.: fragilidade ao calor — descreva como foi contornado <<<
\end{itemize}
...
% ==================================================================
%  4.2.2  LABIRINTO MODULAR DE TESTES (4×4)
% ==================================================================

\subsection{Labirinto Modular de Testes (4×4)}

% ------------------------------------------------------------------
% TEXTO — Visão geral do labirinto de testes
% Explique o objetivo do labirinto de testes e que ele deve reproduzir
% as condições da competição (células 18 cm, paredes 5 cm × 1,2 cm,
% piso preto, paredes brancas com topos vermelhos).
% ------------------------------------------------------------------

% >>> ESCREVA AQUI: visão geral do labirinto de testes (3–5 linhas) <<<

% ------------------------------------------------------------------
% FIGURA — Render / foto do labirinto completo montado
% ------------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figuras/labirinto_completo}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/labirinto_modulo_cad}
        \caption*{(a) Modelo CAD do módulo}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/labirinto_modulo_foto}
        \caption*{(b) Módulo fabricado}
    \end{minipage}
    \caption{Módulo de parede do labirinto. Altura: 50 mm;
             espessura: 12 mm; comprimento: 180 mm.}
    \label{fig:labirinto_modulo}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Detalhe do sistema de encaixe entre os módulos de parede.
             A folga projetada de ??? mm garante montagem sem ferramentas
             e desmontagem rápida para reconfiguração do labirinto.}
    \label{fig:labirinto_encaixe}
\end{figure}

% ------------------------------------------------------------------
% TABELA — Dimensões do labirinto conforme as regras da competição
% ------------------------------------------------------------------
...
\subsubsection*{Justificativa do Material — Madeira (???: MDF / Compensado / Pinus)}

% >>> ESCREVA AQUI: justificativa técnica da madeira escolhida <<<
% Tópicos obrigatórios (conforme critérios da issue):
%  - Baixo custo e ampla disponibilidade
%  - Rigidez e durabilidade suficientes para os testes
%  - Facilidade de corte (serra, laser) e acabamento (pintura)
%  - Massa final estimada do labirinto completo
%  - Comparação breve com alternativas descartadas (alumínio, acrílico etc.)

O material selecionado para as paredes e base do labirinto de testes
foi \textbf{???} pelos seguintes motivos:
...
\begin{itemize}
    ...
    \item \textbf{Facilidade de fabricação e acabamento:}
          % >>> descreva aqui (corte a laser, lixamento, pintura) <<<
...
% >>> ESCREVA AQUI: como foram aplicadas as cores (paredes brancas,
%     topos vermelhos, piso preto) e qual tinta/processo foi utilizado <<<
